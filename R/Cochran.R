...
#' @param formula defines block and group variables of the order lhs ~ group | block
#' @param type how to compute p? exact or montecarlo
#'
#' @return list with statistic and corresponding one sided p value
#' @export
#'
#' @examples
#' autos <- matrix(
...
#' cochran_test(value ~ group | family,
#'   formula = value ~ group | family,
#'   type = "montecarlo"
#' )
cochran_test <- function(.data,
                         formula,
                         type = c("exact", "montecarlo"),
                         n_samples = 10000) {
  # check inputs
  match.arg(type)
  ...
  get_qs <- function(n) {
    ...
    compute_q({
      aggregate(lhs, list(group_vec), FUN = sum) %>% pull(x)
    }^2 %>% sum())
  }
...
#'
#' @param .f get q function that takes one permutation number as input
#' @param type either "exact" or "montecarlo"
#' @param n_samples number of draws for montecarlo method
#' @param n_permutations number of possible permutations
#'
...
get_q_vector <- function(.f, type, n_samples, n_permutations) {
  ...
  if (type == "montecarlo") {
    qs <- tryCatch(purrr::map_dbl(sample(1:n_permutations, size = 10000, replace = TRUE), .f = .f, .progress = TRUE),
      error = function(e) {
        warning(paste("the following error occured:",
          e,
...
#'   mutate(family = row_number()) %>%
#'   pivot_longer(V1:V4, names_to = "group")
#' create_q_f(value ~ group | family, formula = value ~ group | family)
